...
if [[ "${NO_BUILD}" != "1" ]]; then
  if ! command -v nvcc >/dev/null 2>&1; then
    echo "nvcc not found. Install CUDA Toolkit or provide a prebuilt matmul_cuda." >&2
    exit 1
  fi
  make -C "${MATMUL_DIR}" matmul_cuda
fi
...
for i in $(seq 1 "${RUNS}"); do
  if [[ -n "${SEED_HEX}" ]]; then
    output=$("${MATMUL_DIR}/matmul_cuda" --upow --seed-hex "${SEED_HEX}" --output "${MATMUL_DIR}/solution.bin")
  else
    output=$("${MATMUL_DIR}/matmul_cuda" --upow --seed-path "${SEED_BIN}" --output "${MATMUL_DIR}/solution.bin")
  fi
  echo "${output}"
