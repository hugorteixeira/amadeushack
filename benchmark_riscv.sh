...
if [[ "${NO_BUILD}" != "1" ]]; then
  ...
  if [[ "${TT_BAREMETAL}" == "1" ]]; then
    ...
    printf '#define TT_SEED_HEX "%s"\n#define TT_CPU_HZ %s\n#define TT_USE_RDCYCLE %s\n#define TT_PROGRESS %s\n' \
      "${SEED_HEX}" "${TT_CPU_HZ}" "${TT_USE_RDCYCLE}" "${TT_PROGRESS}" > "${SEED_HEADER}"
...
for i in $(seq 1 "${RUNS}"); do
  ...
  end_ns=$(now_ns)
  host_elapsed_ms=0
  if is_number "${start_ns}" && is_number "${end_ns}" && [[ "${start_ns}" != "0" && "${end_ns}" != "0" ]]; then
  ...
  if [[ -z "${elapsed}" || -z "${gflops}" ]]; then
    ...
    if [[ -n "${elapsed_cycles}" ]]; then
      calc=""
      if command -v python3 >/dev/null 2>&1; then
        calc=$(ELAPSED_CYCLES="${elapsed_cycles}" TT_CPU_HZ="${TT_CPU_HZ}" python3 - <<'PY'
import os
